...
class AddMemoViewModel(
    ...
    suspend fun createStatement() {
        val memo = if (stateMemo.value?.trim().isNullOrEmpty()) "짤랑짤랑~" else stateMemo.value?.trim()
            ?: "짤랑짤랑~"
        val statementData = Statement(
            date = stringToDate(stateDate.value ?: getDateForNow()),
            content = memo,
            amount = applyType(getAmount()),
