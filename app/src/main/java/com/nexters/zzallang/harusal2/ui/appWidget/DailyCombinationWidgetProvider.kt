...
@KoinApiExtension
class DailyCombinationWidgetProvider : AppWidgetProvider(), KoinComponent {
	companion object {
		const val DAILY_COMBINATION_REQUEST_CODE = 104
	}
	...
	override fun onReceive(context: Context?, intent: Intent?) {
		...
		val ids = AppWidgetManager.getInstance(context).getAppWidgetIds(ComponentName(context, DailyCombinationWidgetProvider::class.java))
		val widget = DailyCombinationWidgetProvider()
	...
	private fun RemoteViews.setOnClickOpenApp(context: Context?, appWidgetId: Int) {
		val pendingIntent = PendingIntent.getActivity(
			context,
			DAILY_COMBINATION_REQUEST_CODE + appWidgetId,
	...
	private fun RemoteViews.setOnClickRefresh(context: Context?, appWidgetId: Int) {
		val intent = Intent(context, DailyCombinationWidgetProvider::class.java).apply {
